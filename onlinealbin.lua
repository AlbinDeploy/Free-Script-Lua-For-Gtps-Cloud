...
local COUNTRY_MAP = {}
for _, v in ipairs(COUNTRY_NAME_MAP) do
    COUNTRY_MAP[v[1]:lower()] = v[2]
end

local DEVICE_ORDER = { "Android", "IOS", "PC", "MacBook", "Linux", "Unknown-Device" }
...
local function showServerStatsDialog(player)
    ...
    local maxOnline = 3000
    local currentOnline = cache.onlineCount
    if currentOnline > maxOnline then currentOnline = maxOnline end
    ...
    dialog = dialog .. "add_progress_bar|  `oServer: " .. cache.serverName .. "``|small|  `oOnline:``|" .. tostring(currentOnline) .. "|" .. tostring(maxOnline) .. "|(" .. tostring(currentOnline) .. "/" .. tostring(maxOnline) .. ")|-3669761|\n"
    dialog = dialog .. "add_smalltext|`oUptime: `2" .. formatUptime(cache.uptime) .. "|left|\n"
    dialog = dialog .. "add_smalltext|`o────────────────────────────|\n"
    ...
    for _, deviceName in ipairs(DEVICE_ORDER) do
        ...
        dialog = dialog .. "add_smalltext|`o" .. deviceName .. " Users: `2" .. v .. "|\n"
    end
    ...
    if #countryPairs > 0 then
        ...
        for _, entry in ipairs(countryPairs) do
            ...
            dialog = dialog .. "add_smalltext|`w" .. name .. " - `2" .. tostring(count) .. " `$User Online|left|\n"
            dialog = dialog .. "reset_placement_x|\n"
            displayed = displayed + 1
        end
        if #countryPairs > maxDisplay then
            dialog = dialog .. "add_smalltext|`oAnd more... (" .. tostring(#countryPairs - maxDisplay) .. ")|\n"
        end
        dialog = dialog .. "add_smalltext|`o────────────────────────────|\n"
    end
